...
<div class="center-text">
    ...
    <div class="col-md-6">
        <form action="login" method="POST">
            ...
            <input type="submit" value="Login" class="login-submit btn btn-uergs" />

            <p class='login-prompts'>
            @if (env('POLR_ALLOW_ACCT_CREATION') == true)
                <small>Não possui uma conta? <a href='{{route('signup')}}' class="text-uergs">Cadastre-se</a></small>
            @endif

            @if (env('SETTING_PASSWORD_RECOV') == true)
                <small>Esqueceu sua senha? <a href='{{route('lost_password')}}' class="text-uergs">Recuperar senha</a></small>
            @endif
            </p>
        </form>
    </div>
