        ...
        controller: function ($scope, $element, $timeout) {
            ...
            $scope.saveChanges = function () {
                ...
                apiCall('admin/edit_link_long_url', {
                    ...
                    toastr.success('O link foi atualizado.', 'Sucesso')
                }, function(err) {
                    toastr.error('O novo formato da URL não é válido.', 'Erro');
                });
            };
        ...
        controller: function ($scope, $element, $timeout) {
            ...
            $scope.updateAPIQuota = function() {
                apiCall('admin/edit_api_quota', {
                    ...
                    toastr.success("Cota alterada com sucesso.", "Sucesso");
                });
            };
...
polr.controller('AdminCtrl', function($scope, $compile, $timeout) {
    ...
    */
    $scope.toggleUserActiveStatus = function($event, user_id) {
        ...
        apiCall('admin/toggle_user_active', {
            ...
            var text = (new_status == 1) ? 'Ativo' : 'Inativo';
            el.text(text);
    ...
    $scope.addNewUser = function($event) {
        ...
        if (!$scope.checkNewUserFields()) {
            toastr.error("Os campos devem ser preenchidos.", "Erro");
            return false;
        }
        ...
        apiCall('admin/add_new_user', {
            ...
            toastr.success("Usuário " + $scope.newUserParams.username + " criado com sucesso.", "Sucesso");
            $('#new-user-form').clearForm();
            $scope.datatables['admin_users_table'].ajax.reload();
        }, function () {
            toastr.error("Um erro ocorreu ao criar o usuário.", "Erro");
        });
    }

    // Delete user
    $scope.deleteUser = function($event, user_id) {
        var delete_user_confirm = window.confirm('Tem certeza que quer remover esse usuário?');
    ...
    $scope.changeUserRole = function(role, user_id) {
        apiCall('admin/change_user_role', {
            ...
            toastr.success("Cargo do usuário alterado com sucesso.", "Sucesso");
        });
    };
    ...
    */
    ...
    $scope.deleteLink = function($event, link_ending) {
        var delete_link_confirm = window.confirm('Tem certeza que quer remover esse link?');

        if (delete_link_confirm) {
            apiCall('admin/delete_link', {
                'link_ending': link_ending,
            }, function(new_status) {
                toastr.success('Link removido com sucesso.', 'Sucesso');
                $scope.reloadLinkTables();
            });
        }
    };
    ...
    $scope.toggleLink = function($event, link_ending) {
        ...
        apiCall('admin/toggle_link', {
            ...
            toastr.success(curr_action + " ocorreu com sucesso.", "Sucesso");
