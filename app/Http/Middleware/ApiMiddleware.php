    ...
    protected static function getApiUserInfo(Request $request) {
        ...
        if (!$api_key) {
            ...
            if (env('SETTING_ANON_API')) {
                ...
                throw new ApiException('AUTH_ERROR', 'Token de autenticação requirido.', 401, $response_type);
            }
            $user = (object) [
                'username' => $username,
                'anonymous' => true
            ...
            if (!$user) {
                throw new ApiException('AUTH_ERROR', 'Token de autenticação inválido.', 401, $response_type);
            }
        ...
        if ($api_limit_reached) {
            throw new ApiException('QUOTA_EXCEEDED', 'Cota excedida.', 429, $response_type);
        }
        return $user;
    }
