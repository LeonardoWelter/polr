...
class ApiLinkController extends ApiController {
    ...
    public function shortenLinksBulk(Request $request) {
        ...
        if ($response_type != 'json') {
            throw new ApiException('JSON_ONLY', 'Somente respostas JSON-encoded estão disponíveis nesse endpoint.', 401, $response_type);
        }

        $links_array_raw_json = json_decode($request_data, true);

        if ($links_array_raw_json === null) {
            throw new ApiException('INVALID_PARAMETERS', 'JSON inválido.', 400, $response_type);
        }
    ...
    public function lookupLink(Request $request) {
        ...
        if ($link['secret_key']) {
            if ($url_key != $link['secret_key']) {
                throw new ApiException('ACCESS_DENIED', 'Parâmetros inválidos ou não estão presentes.', 401, $response_type);
            }
        }
