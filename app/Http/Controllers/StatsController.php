...
class StatsController extends Controller {
    ...
    public function displayStats(Request $request, $short_url) {
        ...
        if ($validator->fails() && !session('error')) {
            ...
            return redirect()->back()->with('error', 'Intervalo de data inválido.');
        }
        ...
        if (Carbon::parse($right_bound)->gt(Carbon::now()) && !session('error')) {
            ...
            return redirect()->back()->with('error', 'Início do intervalo de data não pode ser no futuro.');
        }

        if (!$this->isLoggedIn()) {
            return redirect(route('login'))->with('error', 'Faça login para verificar as estatísticas do link.');
        }
        ...
        if ($link == null) {
            return redirect(route('admin'))->with('error', 'Não é possível mostrar estatísticas para links que não existentes.');
        }
        if (!env('SETTING_ADV_ANALYTICS')) {
            return redirect(route('login'))->with('error', 'Habilite Analytics avançado para acessar essa página.');
        }

        $link_id = $link->id;

        if ( (session('username') != $link->creator) && !self::currIsAdmin() ) {
            return redirect(route('admin'))->with('error', 'Você não possui permissão para ver as estatísticas desse link.');
        }
        ...
        try {
            ...
            if (!session('error')) {
                // Do not flash error if there is already an error flashed
                return redirect()->back()->with('error', 'Intervalo de data inválido.
                    O final do intervalo precisa ser mais recente que o ínicio.');
            }
        }
