...
class UserController extends Controller {
    ...
    public function performLogin(Request $request) {
        ...
        if ($credentials_valid != false) {
            ...
            return redirect('login')->with('error', 'Senha inválida ou conta desativada. Tente novamente.');
        }
    }

    public function performSignup(Request $request) {
        if (env('POLR_ALLOW_ACCT_CREATION') == false) {
            return redirect(route('index'))->with('error', 'Cadastro de novas contas está desativado.');
        }
        ...
        if (env('POLR_ACCT_CREATION_RECAPTCHA')) {
            ...
            if (!$recaptcha_resp->isSuccess()) {
                return redirect(route('signup'))->with('error', 'Você precisa completar o reCAPTCHA para cadastrar-se.');
            }
        }
        ...
        if (env('SETTING_RESTRICT_EMAIL_DOMAIN')) {
            ...
            if (!in_array($email_domain, $permitted_email_domains)) {
                return redirect(route('signup'))->with('error', 'O domínio do seu email não é permitido na criação de novas contas.');
            }
        }
        ...
        if ($user_exists || $email_exists) {
            ...
            return redirect(route('signup'))->with('error', 'Seu email ou nome de usuário já existe. Tente novamente.');
        }
        ...
        if ($acct_activation_needed == false) {
            ...
            $active = 1;
            $response = redirect(route('login'))->with('success', 'Obrigado por cadastrar-se! Você já pode fazer login.');
        }
        else {
            // email activation is necessary
            $response = redirect(route('login'))->with('success', 'Obrigado por cadastrar-se! Confirme seu email para continuar.');
            $active = 0;
        }
        ...
        if ($acct_activation_needed) {
            Mail::send('emails.activation', [
                ...
                $m->to($user->email, $user->username)->subject(env('APP_NAME') . ' ativação de conta');
            });
        }
    ...
    public function performSendPasswordResetCode(Request $request) {
        if (!env('SETTING_PASSWORD_RECOV')) {
            return redirect(route('index'))->with('error', 'Recuperação de senha está desativada.');
        }
        ...
        if (!$user) {
            return redirect(route('lost_password'))->with('error', 'Esse email não está associado com um usuário.');
        }
        ...
        Mail::send('emails.lost_password', [
            ...
            $m->to($user->email, $user->username)->subject(env('APP_NAME') . ' recuperação de senha');
        });

        return redirect(route('index'))->with('success', 'Email de recuperação de senha enviado. Verifique sua caixa de entrada para mais detalhes.');
    }
    ...
    public function performActivation(Request $request, $username, $recovery_key) {
        ...
        if (UserHelper::userResetKeyCorrect($username, $recovery_key, true)) {
            ...
            return redirect(route('login'))->with('success', 'Conta ativada! Você já pode fazer login.');
        }
        else {
            return redirect(route('index'))->with('error', 'Nome de usuário ou chave de ativação incorreto.');
        }
    }
    ...
    public function performPasswordReset(Request $request, $username, $recovery_key) {
        ...
        if (UserHelper::userResetKeyCorrect($username, $recovery_key)) {
            ...
            UserHelper::resetRecoveryKey($username);
            return redirect(route('login'))->with('success', 'Senha alterada. Você já pode fazer login.');
        }
        else {
            return redirect(route('index'))->with('error', 'Nome de usuário ou chave de recuperação inválida.');
        }
